...
public class SwaggerConfig {
	...
	private ApiInfo apiInfo() {
		Contact contact = new Contact("蒋宁善", "http://www.4399.com/", "ester1753291617@163.com");
...
//	      .paths(PathSelectors.ant("/kuang/**"))
	...
	public Docket docket1(Environment environment) {
		...
		return new Docket(DocumentationType.SWAGGER_2).groupName("蒋宁善").apiInfo(apiInfo()).enable(b) // 配置是否启用Swagger，如果是false，在浏览器将无法访问
				.select()// 通过.select()方法，去配置扫描接口,RequestHandlerSelectors配置如何扫描接口
//		      .apis(RequestHandlerSelectors.basePackage("main.controller"))
				// 配置如何通过path过滤,即这里只扫描请求以/kuang开头的接口
//		      .paths(PathSelectors.ant("/kuang/**"))
